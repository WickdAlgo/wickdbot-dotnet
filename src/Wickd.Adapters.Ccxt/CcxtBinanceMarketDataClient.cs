...
namespace Wickd.Adapters.Ccxt;

/// <summary>
/// Fetches Binance USD-M historical candles through CCXT while preserving Wickd's Binance exchange identity.
/// </summary>
public sealed class CcxtBinanceMarketDataClient : IMarketDataClient
{
    /// <summary>
    /// Wickd exchange ID supported by this client.
    /// </summary>
    public const string WickdExchangeId = "binance";

    /// <summary>
    /// CCXT exchange ID used for Binance USDT perpetual markets.
    /// </summary>
    public const string CcxtExchangeId = "binanceusdm";

    /// <summary>
    /// Maximum OHLCV candles requested per CCXT page.
    /// </summary>
    public const int FetchLimit = 1000;

    /// <inheritdoc />
    public string ExchangeId => WickdExchangeId;
    ...
    /// <summary>
    /// Resolves the CCXT adapter ID for a configured Wickd Binance market.
    /// </summary>
    /// <param name="market">Configured market identity.</param>
    /// <returns>The CCXT exchange ID.</returns>
    /// <exception cref="WickdDataException">Thrown when the market is not supported by this client.</exception>
    public static string ResolveCcxtExchangeId(MarketDefinition market)
    {
        if (market.ExchangeId != WickdExchangeId)
        {
            throw new WickdDataException(
    ...
    /// <summary>
    /// Converts a CCXT OHLCV record into Wickd's exchange candle contract.
    /// </summary>
    /// <param name="ohlcv">CCXT OHLCV record.</param>
    /// <returns>The converted exchange candle.</returns>
    /// <exception cref="WickdDataException">Thrown when CCXT omits required OHLCV fields.</exception>
    public static ExchangeCandle ConvertOhlcv(OHLCV ohlcv)
    {
        if (ohlcv.timestamp is null)
        {
            throw new WickdDataException("CCXT OHLCV candle is missing timestamp.");
        }
    ...
    /// <exception cref="WickdDataException">Thrown when CCXT returns a null page.</exception>
    public static IReadOnlyCollection<OHLCV> RequireOhlcvPage(
    ...
    public static int CountNextFetchLimit(
    ...
    /// <exception cref="WickdDataException">Thrown when the field is missing or non-finite.</exception>
    private static decimal ConvertRequiredDecimal(double? value, string fieldName)
    {
        if (value is null)
        {
            throw new WickdDataException($"CCXT OHLCV candle is missing {fieldName}.");
        }

        if (double.IsNaN(value.Value) || double.IsInfinity(value.Value))
        {
            throw new WickdDataException($"CCXT OHLCV candle has non-finite {fieldName}.");
        }
