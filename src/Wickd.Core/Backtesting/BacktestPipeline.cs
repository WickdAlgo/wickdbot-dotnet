...
internal sealed class BacktestPipeline
{
    ...
    /// <param name="settings">Validated Wickd settings.</param>
    /// <param name="cancellationToken">Cancellation token for file I/O.</param>
    /// <returns>The Phase 3 backtest pipeline result.</returns>
    /// <exception cref="WickdDataException">Thrown when replay or structure journal writing fails.</exception>
    ...
    /// <exception cref="WickdDataException">Thrown when the run ID is empty, path-like, or contains unsupported characters.</exception>
    internal static void ValidateRunId(string runId)
    {
        ...
        foreach (var character in runId)
        {
            ...
            {
                throw new WickdDataException(CreateInvalidRunIdMessage());
            }
