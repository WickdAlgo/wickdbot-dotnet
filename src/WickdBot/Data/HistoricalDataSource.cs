...
/// </summary>
internal sealed class HistoricalDataSource
{
    /// <summary>
    /// Market data clients keyed by WickdBot exchange ID.
    /// </summary>
    ...
    /// <summary>
    /// Creates the production historical data source for currently supported exchanges.
    /// </summary>
    /// <returns>The configured production historical data source.</returns>
    internal static HistoricalDataSource CreateDefault()
    {
    ...
    /// <exception cref="WickdBotDataException">Thrown when the request, cache, exchange, or candle data is invalid.</exception>
    internal async Task<HistoricalDataResult> LoadOrFetchAsync(
        RunRequest request,
    ...
    /// <exception cref="WickdBotDataException">Thrown when the cache is missing or invalid.</exception>
    internal async Task<CandleReplayResult> ReplayAsync(
        RunRequest request,
        string runId,
        CancellationToken cancellationToken = default)
    {
    ...
    /// </summary>
    /// <param name="exchangeId">Configured WickdBot exchange ID.</param>
    /// <returns>The market data client.</returns>
    /// <exception cref="WickdBotDataException">Thrown when no Phase 2 client supports the exchange.</exception>
    ...
    private static void ValidateCachedHistoricalCandle(RunRequest request, CandleEvent candle)
    {
        ...
        {
            throw new WickdBotDataException("Candle cache must contain historical-source candles.");
        }

        if (candle.RunId is not null)
        {
            throw new WickdBotDataException("Historical candle cache must not contain run IDs.");
        }
    ...
    /// <summary>
    /// Converts one exchange candle into a normalized WickdBot historical candle.
    /// </summary>
    /// <param name="request">Resolved historical run request.</param>
    /// <param name="candle">Raw exchange candle.</param>
    /// <returns>The WickdBot historical candle.</returns>
        ...
        IEnumerable<CandleEvent> candles)
    {
        ...
        {
            throw new WickdBotDataException($"Candle data is invalid: {ex.Message}", ex);
        }
    }
