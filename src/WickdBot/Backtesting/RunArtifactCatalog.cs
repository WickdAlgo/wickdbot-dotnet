...
internal static class RunArtifactCatalog
{
    ...
    /// <exception cref="WickdBotDataException">Thrown when the runs root cannot be inspected.</exception>
    internal static IReadOnlyList<RunArtifact> List(string runsRoot)
    {
        if (string.IsNullOrWhiteSpace(runsRoot))
        {
            throw new WickdBotDataException("Runs root is required.");
        }
    ...
    /// <exception cref="WickdBotDataException">Thrown when the run ID is invalid or deletion fails.</exception>
    ...
    /// <exception cref="WickdBotDataException">Thrown when the target escapes the root path.</exception>
